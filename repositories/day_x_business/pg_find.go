...
func Pg_Find(idbusiness int, idcountry int) ([]models.Pg_R_Schedule, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&schedule.IDSchedule, &schedule.Name, &schedule.Starttime, &schedule.Endtime, &schedule.Available)
